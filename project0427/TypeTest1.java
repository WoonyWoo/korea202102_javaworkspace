...
class TypeTest1{
	public static void main(String[] args){
		...
		정수의 연산은 int 기준으로 최적화 되어 있기 때문에, 연산시
		int 이하의 자료형들은 컴파일러에 의해 자동으로 자료형을 int형으로 반환
		해 버린다... 이러한 현상을 개발자에 의해서가 아니라 시스템에 의한
		형변환이므로 '자동형변환' 이라 한다	
		참고) 왜 int 이하만 적용되는 이유는? 손실을 발생시키면서까지 속도를 올릴 이유는 없다
		*/
		...
		//큰 자료형을 기준으로 형변환된다!!
		//int result2 = x+k; 
		//int result2 = (int)(x+k); 손실을 감안하면서까지 int로 강제 형변환
	}
}