...
public class BasicServer {
	ServerSocket server; //소켓 서버 구축을 위한 서버 객체 (접속을 받아들이는 객체)
	...
	public BasicServer() {
		try {
			server = new ServerSocket(7777); //7777 포트로 접속자를 기다리겠다!!
			System.out.println("서버 객체 생성!!");
			
			//접속자를 감지하는 메서드 호출
			Socket socket = server.accept(); //접속자의 접속으 기다리다가, 접속이 발생할 경우 클라이언트를 받아들임!!
			//접속자가 발견되면, 이 시점부터는 접속자인 클라이언트와 대화를 나눌 수 있으므로, 대화를 주고받는 전화기에서
			//반환받는다!!
