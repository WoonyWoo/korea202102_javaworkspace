...
/*js와 마찬가지로, 객체의 인스턴스를 생성할 때 초기화 작업을 담당하게 될
...
public class Hero{
	...
	//다음과 같이 개발자가 초기화의 의지를 갖고, 생성자를 정의하면 시스템의 의한 컴파일러에
	//의한 디폴트 생성자는 정의되지 않는다..따라서 현재 클래스 내의 생성자의 수는 1개이다!!
	...
	//다음에 오는 생성자 호출 시 에러가 발생하게 되므로,, sun에서는 이를 방지하기 위해
	//개발자가 정의해야 할 생성자를 살짝 디폴트 생성자라는 역할로 정의해 놓는다...
	//즉 시스템에 의해 생성자가 정의된다...이때 시스템에 의한 생성자 정의라는 관여는
	//최대한 관여? 최소한 관여? O, 이때 최소한의 관여만 하게 되므로, 시스템에 의한 디폴트
